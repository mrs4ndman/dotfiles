...
alias spotdl-playlist-flac='spotdl --audio youtube-music --bitrate auto --threads 8 --preload --m3u playlist-name --print-errors --scan-for-songs --format flac --overwrite metadata'
...
alias af='alias | fzf'
alias fv='file=$(fd --hidden | fzf); if [ -n "$file" ]; then nvim $file; fi'
...
alias v='nvim'
alias oil='nvim "+Oil"'
alias nvim-rebuild='pushd ~/git-clone-zone/neovim && make CMAKE_BUILD_TYPE=RelWithDebInfo && pushd build && cpack -G DEB && sudo dpkg -i nvim-linux-x86_64.deb && nvim --version && popd && popd'
