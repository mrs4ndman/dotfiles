...
wk.register({
    ...
    ["<leader>csn"] = { "<Plug>(cokeline-switch-next)", "Switch with next buffer", { silent = true } },
    ["<leader>csp"] = { "<Plug>(cokeline-switch-prev)", "Switch with next buffer", { silent = true } },
})
...
for i = 1, 9 do
    ...
    vim.keymap.set('n', ('<leader>s%s'):format(i), ('<Plug>(cokeline-switch-%s)'):format(i), { desc = "Switch with buffer x", silent = true })
end
